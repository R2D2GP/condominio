document.addEventListener('DOMContentLoaded', () => {
  ...
  const closeModal = (modal) => {
    ...
    // Generic reset for all inputs and selects as a fallback.
    const textInputs = modal.querySelectorAll('input[type="text"], textarea');
    const selects = modal.querySelectorAll('select');

    textInputs.forEach(input => {
        // A specific rule for amount fields, otherwise clear.
    ...
    // Explicitly ensure specific fields are reset, which is safer.
    if (modal.id === 'ingreso-modal') {
        const monto = document.getElementById('ingreso-monto');
        if (monto) monto.value = '0.00';
        const depto = document.getElementById('ingreso-departamento');
        if (depto) depto.value = '';
        const inquilino = document.getElementById('ingreso-inquilino');
        if (inquilino) inquilino.value = '';
        const desc = document.getElementById('ingreso-descripcion');
        if (desc) desc.selectedIndex = 0;
    }

    if (modal.id === 'gasto-modal') {
        const monto = document.getElementById('gasto-monto');
        if (monto) monto.value = '0.00';
        const desc = document.getElementById('gasto-descripcion');
        if (desc) desc.value = '';
        const notas = document.getElementById('gasto-notas');
  ...
  const renderMovements = () => {
    ...
    if (movements.length === 0) {
      ...
      movements.forEach(movement => {
        ...
        row.innerHTML = `
          <td class="p-4 text-gray-600 dark:text-gray-300 text-sm whitespace-nowrap">${formatDate(movement.date)}</td>
          <td class="p-4">
